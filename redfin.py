...
# Code borrowed from https://github.com/yuanfanz/Redfin.
...
class RedFin():
    def __init__(self, start_url):
        #self.start_url = 'https://www.redfin.com/city/17675/CA/Santa-Clara/'
    ...
    def get_property_data(self, output_json_file='redfin_output.json'):
        count = 0
        for property_url in tqdm.tqdm(self.property_urls):
            #print('page: ', property_url)
            self.output_data.append(self.get_property_page(property_url))
            #print('len = ', len(self.output_data))
            count += 1
            #print('finished page ' + str(count))
            open(output_json_file, 'w').write(json.dumps(self.output_data, indent=4))
    ...
    def parse_property_page(self, page_source, property_url):
        ...
        try:
            property_data['restimate'] = self.soup.find('span', attrs={'data-rf-test-id': 'avmLdpPrice'}).find('span', attrs={'class': 'value'}).get_text()
        except:
            property_data['restimate'] = 'N/A';print('restimate not found')
        try:
        ...
        try:
            detail_list = self.soup.find('div', attrs={'class': 'keyDetailsList'})
            property_data['year_built'] = detail_list.find('span', attrs={'class': 'header font-color-gray-light font-weight-roman'}, text='Built').nextSibling.get_text()
            property_data['mls_number'] = detail_list.find('span', attrs={'class': 'header font-color-gray-light font-weight-roman'}, text='MLS#').nextSibling.get_text()
            property_data['property_type'] = detail_list.find('span', attrs={'class': 'header font-color-gray-light font-weight-roman'}, text='Property Type').nextSibling.get_text()
...
#                'span', attrs={'class': 'value'}).get_text()
            ...
            property_data['mls_number'] = 'N/A'
            property_data['property_type'] = 'N/A'
            print('year_built, mls_number, property_type not found')
#            property_data['year_built'] = 'N/A';print('year_built not found')
        try:
            property_data['days_on_redfin'] = self.soup.find('div',
                                                             attrs={'class': 'more-info'}).find('span', text='On Redfin: ').nextSibling.get_text()
        except:
        ...
        try:
            property_data['urgent_msg'] = self.soup.find('div',
                                                     attrs={'data-rf-test-name': 'HomeDetailsBanner',
                                                     'class': 'HomeDetailsBanner urgent'}).find('div', attrs={'class': 'textContainer'}).get_text()
        except:
            property_data['urgent_msg'] = '';#print('urgent msg not found')
        ...
        try:
            ...
            for category in self.soup.find('div', attrs={'class': 'amenities-container'}).children:
                ...
                key = str(category.contents[0].encode('utf-8'))
            ...
            print('missing summary data for ', property_url)
